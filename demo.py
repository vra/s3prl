...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="choices of task, 'infer' means extract embedding of a wave, 'export' means export pytorch model to onnx",
    )
    parser.add_argument(
        "-m",
        "--ckpt_path",
        default=None,
        help="path of wavlm_large pth path, can be downloaded from huggingface",
    )
    parser.add_argument(
        "-i",
        "--input_audio_path",
...
if __name__ == "__main__":
    ...
    if args.task == "infer":
        assert (
            args.input_audio_path is not None
        ), "when task set to 'infer', you must give a path of audio"
        embed = extractor.extract_embedding(args.input_audio_path)
        print("embedding's mean:", embed.mean())

    if args.task == "export":
        assert (
            args.onnx_save_path is not None
        ), "when task set to 'export', you must give a path to save onnx model"
        extractor.export_onnx(args.onnx_save_path)
        print("onnx saved to:", args.onnx_save_path)
